...
function show_usage {
print "usage: mtree-copy-retentionlock.ksh -o <full or partial> -d <Data Domain> -u <User> -s <Source Directory> -m <Source Mtree> -t <Target Directory> -n <Target Directory> -r <Comparing Days> -l <yes if retention lock should be added to the file> -k <retention lock days>" 
...
if [[ $full = "full" || $full = "Full" || $full = "FULL" ]]; then
     ...
     if test $ret; then
        find $sdir -type f -mtime -$ret|  grep -v "snapshot" > $DIR/file-in-sdir 
     else
...
if [[ $full = "full" || $full = "Full" || $full = "FULL" ]]; then
     ...
     find $tdir -type f | grep -v "snapshot" > $DIR/file-in-tdir 
else
     if test $ret; then
...
do
    ...
    if [ $? -ne 0 ]; then
         ...
         echo $locktime
         echo "touch -a -t $locktime $tdir/$bfile" >> $DIR/setretention.ksh
         let numline=$numline+1
#        echo $numline
    else
         echo "$bfile is already in $tdir directlry, skip"    
